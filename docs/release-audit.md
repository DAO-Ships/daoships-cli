...
## Alpha.2 completeness review

The ABI inventory still covers 353 functions across 17 contracts and 88 commands. The review found that the CLI applied action-success checks to intentional defeated-proposal closure and unnecessarily fetched indexed calldata before preparing it. Named commands, generic writes and journal recovery now accept that closure while preserving errors for failed execution and retention vetoes. Three regressions cover exact events, wrong emitters/IDs, missing calldata for Ready proposals, and closure recovery without rebroadcast. The SDK already supports both processing branches.
